// Convert the methodology summary markdown to a Word doc so the user
// can read it without a markdown viewer.
...
const MD_PATH = path.join(__dirname, "..", "..", "documents", "PSLE 华文词汇分析 — 方法与发现总结.md");
const OUT_PATH = path.join(__dirname, "..", "..", "documents", "PSLE 华文词汇分析 — 方法与发现总结.docx");

function parseInline(text: string): TextRun[] {
  // Handle **bold**, *italic*, `code`. Greedy but good enough for our doc.
  const runs: TextRun[] = [];
  const re = /(\*\*[^*]+\*\*|\*[^*]+\*|`[^`]+`)/g;
  let last = 0;
...
(async () => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Table detection: a line `| ... | ... |` followed by `| --- | --- |`
    if (/^\|.*\|\s*$/.test(line)) {
