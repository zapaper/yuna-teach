...
- For 语文应用 MCQ (一, Q1-15) — PRESERVE EVERY visual marking on the tested word/phrase, in BOTH stem and EACH option. This section's ENTIRE POINT is testing recognition of a specific word/phrase, which is ALWAYS visually emphasised in print. If you output plain text for the tested phrase, the question becomes unanswerable — the student can no longer see WHICH word is being tested.
  ...
  - Some Q1-15 questions DO have an emphasised tested phrase (近义词 / 反义词 / 词语用法 style — the printed text shows the phrase in bold and/or underline) and some DO NOT (词语搭配 with a blank, 标点 punctuation choice, 句子排列 sentence-ordering — plain text throughout). Do not invent emphasis where the printed page shows none. The rule is preservation: if the page shows emphasis, copy it as markdown; if the page shows plain text, leave it plain.
