...
const SUB_TOPIC_FOCUS: Record<string, string> = {
  ...
  "subordinator": "LINKING WORDS — Concession, cause, condition or purpose. Each option below is a valid target pattern; the variant must use one of them and follow that rule exactly. Concession: Although/Even though + clause OR Despite/In spite of + noun phrase OR No matter how/what/where + clause. Cause: Because/Since/As + clause OR Due to/Owing to/On account of + noun phrase. Condition: If + present, will + verb OR Unless (= if not — DO NOT add a second 'not'). Purpose: In order to / So as to + verb OR So that + clause. Across the two variants, vary the sub-pattern (e.g. one concession, one cause) so the student practises the breadth of linking words.",
  "correlative-preference": "PAIR WORDS & PREFERENCE — Either a correlative pair (Both X and Y / Either X or Y / Neither X nor Y / Not only X but also Y / As well as / X or Y) OR a preference form (Would rather + bare verb + than + bare verb / Prefer + V-ing to V-ing / Have a preference for X over Y). CRITICAL: 'Not only' at sentence start FORCES inversion ('Not only DOES/DID/HAS Subject + base verb'). NO 'to' after 'than' with would rather. NO -ing after would rather. Across the two variants, vary kind (one pair-word, one preference) when possible.",
  "participle-clauses": "PARTICIPLE CLAUSES — Joins two actions by reducing the first to a participle. Forms: (a) Having + past participle + (object), (new subject) + verb — sequential, first action finished before second; (b) Bare V-ing (Seeing/Hearing/Fearing/Feeling) + clause — concurrent/sensing; (c) Upon/On/After/Before + V-ing — temporal; (d) Being + adjective/past participle — state/passive. Keep his/her/their possessives intact. Negative form: Not having + V-ed.",
  "substitution-inversion": "SUBSTITUTION & INVERSION — Either substitution (Instead of + V-ing or noun / Except + noun / Much to + possessive + noun) OR negative inversion (Little did / No other / No sooner...than / Hardly...when / Never has/have/had). CRITICAL: Inversion after a negative starter flips subject-verb order: starter + helper (does/did/has/had) + Subject + BASE verb. Across the two variants, prefer covering both flavours (one substitution, one inversion).",
  "noun-phrase": "NOUN PATTERNS — Three flavours of reshaping info about a noun. (a) RELATIVE CLAUSE: who = subject (people), whom = object (people), which = things, whose = possessive. Relative pronoun must sit RIGHT AFTER the noun it describes. Commas on both sides for non-defining clauses; no commas for defining. (b) POSSESSIVE 'S TRANSFORMATION: compress 'X was [adj]' or 'X did Y' into an 's noun phrase (Sally's insincerity, the boy's hard work, Without Mr Ahmad's advice). (c) VERB → NOUN CONVERSION: change action verb into a noun phrase. Common openers: made + (adj) + noun, came to/for + noun, took + noun, had a + noun. Preserve any adverb modifier (adjusted slightly → made slight adjustments). Spelling must be exact and real (conclusion, illness, indecision).",
};
...
{
  "header": {
    "school": "...", "level": "Primary 3" or "Primary 4" or "Primary 5" or "Primary 6", "subject": "...", "year": "...",
...
CRITICAL — Chinese Paper 2 is ONE booklet, never split into Booklet A/B:
Chinese (华文) Paper 2 is always a single continuous booklet covering Q1 through ~Q40. Do NOT split it into "Paper 2 Booklet A" + "Paper 2 Booklet B" entries in the papers[] array — that's an English-paper pattern. Even if the printed paper has a mid-document divider or you see a page that looks like a sub-cover, it's still ONE paper with one entry in papers[]. The sections inside (语文应用 / 短文填空 / 阅读理解 / 完成对话) are the meaningful breakdown, not booklets.

CRITICAL — 阅读理解 A 组 (Q30-Q33) sits CONSECUTIVELY with Q33's OEQ adjacent to Q30-Q32's MCQ:
Section 五 A 组 contains 3 MCQ (Q30-Q32) + 1 OEQ (Q33) on the SAME passage. When you split this into two section entries ("阅读理解 MCQ" for Q30-Q32 and "阅读理解 OEQ" for Q33), the two entries MUST have ADJACENT page ranges — the Q30-Q32 section's endPage should immediately precede or equal Q33's startPage. They also share passagePages — copy the SAME passagePages array to both. Never let the "阅读理解 MCQ" for Q30-Q32 drift to the start of the paper or sit before earlier sections. The order MUST be: ... 完成对话 (Q26-29) → 阅读理解 MCQ (Q30-32, A 组) → 阅读理解 OEQ (Q33, A 组) → 阅读理解 OEQ (Q34-40, B 组).
...
- Sections have CONTINUOUS numbering:
  ...
  * Vocabulary Cloze MCQ Q16-20 — these come AFTER Vocabulary MCQ and have a PASSAGE filling the top half of the page, with Q16-20 below it. CRITICAL: This MUST be a SEPARATE section from Vocabulary MCQ. The key difference: Vocab Cloze has a passage ABOVE the questions, Vocab MCQ does NOT. NEVER combine these two sections. IMPORTANT: The passage may contain numbers (dates, amounts, etc.) — these are NOT question numbers. Only look for question numbers at the FAR LEFT MARGIN (within 5% of the left edge).
...
- For 完成对话 (Grammar-Cloze equivalent): the WORD BANK is a BOX at the top containing 6-10 phrases each labelled with a NUMBER (typically 1, 2, 3 … 8). Copy the word bank as a markdown table — one row with the digit labels, one row with the corresponding phrases:
  ...
  CRITICAL: the WHOLE marker — opening **, (N), the underscores, closing ** — must be on a SINGLE line of output. Do NOT break it across lines even if the printed paper renders the blank above and the "(N)" label below it. Inline both into one continuous marker on the dialogue line. Do NOT repeat the (N) inside the marker. Do NOT emit just \`_\` between the asterisks — always use ________ (eight underscores). The (N) inside the parentheses must be JUST THE DIGITS — never prefix with "Q" or any letter. CORRECT: "**(26)________**" / WRONG: "**(Q26)________**".
...
- For 语文应用 MCQ (一, Q1-15) — PRESERVE EVERY visual marking on the tested word/phrase, in BOTH stem and EACH option. This section's ENTIRE POINT is testing recognition of a specific word/phrase or character, which is ALWAYS visually emphasised in print. If you output plain text for the tested element, the question becomes unanswerable — the student can no longer see WHICH word/character is being tested.
  - Read the printed page carefully. The element being tested is shown with one or more visual emphases. Output it with the matching markdown:
      * Bold only                          → **phrase**
      * Underline only                     → __phrase__
      * Bold + underline                   → **__phrase__**
      * 着重号 (DOT below a character)     → __char__ (treat as underline — the dot marks a single tested character)
  - Pattern by sub-type — DETECT WHICH PATTERN APPLIES and emit emphasis accordingly:
      * Q1-Q2-style "拼音读音" (PINYIN READING) — the STEM is a Chinese sentence with ONE character marked by a 着重号 (a small dot printed DIRECTLY BELOW the character, easy to miss because the dot is small). The 4 OPTIONS are pinyin readings with tone marks (e.g. "(1) zhēn (2) zhèn (3) chèn (4) chēn"). MUST DO:
          – Identify the single dotted character in the stem. Wrap ONLY that character in __ __ underline markup (e.g. "这件衣服样式很__新__。"). Do NOT wrap any other characters — only the one with the dot below it.
          – Pinyin in the options: copy the tone marks EXACTLY as printed (ā á ǎ à, ē é ě è, ī í ǐ ì, ō ó ǒ ò, ū ú ǔ ù, ǖ ǘ ǚ ǜ). A wrong tone mark turns a correct answer into a wrong one. Look at each diacritic carefully — ǎ (caron / "v" shape) ≠ á (acute) ≠ à (grave) ≠ ā (macron / straight line).
          – Do NOT drop the dotted character's emphasis. If the dot is genuinely impossible to see in the scan, fall back to bolding the character whose pinyin would distinguish the options — but state nothing else. Never emit a plain stem for a 拼音 question; that makes it unanswerable.
      * Q3-Q5-style "近义词 / 反义词" — the STEM contains an emphasised word (e.g. "他做事很__马虎__。") and the 4 options are plain word candidates. The emphasised word in the STEM must keep its markup. Options may be plain.
  ...
  - Some Q1-15 questions DO have an emphasised tested phrase or 着重号 character (拼音 / 近义词 / 反义词 / 词语用法 style) and some DO NOT (词语搭配 with a blank, 标点 punctuation choice, 句子排列 sentence-ordering — plain text throughout). Do not invent emphasis where the printed page shows none. The rule is preservation: if the page shows emphasis (including a dot below a character), copy it as markdown; if the page shows plain text, leave it plain.
  - When emphasis IS present, copy it on EVERY occurrence — never strip emphasis from an option just because three other options also carry it (Q13-15 "选出正确的一句" pattern). Within a single phrase, if you can see it's emphasised but can't tell whether the print is bold OR underline OR both, default to **__phrase__** (the safest, fully-emphasised form) rather than plain text.
...
- For CLOZE sections:
  ...
  * This cloze formatting applies ONLY to Grammar Cloze and Comprehension Cloze sections.
    ...
    CRITICAL FOR VOCAB CLOZE MCQ — INCLUDE THE OPTIONS BLOCK:
    After the passage, on a NEW LINE, you MUST emit each question's options block exactly as printed on the page. Use this canonical shape, one question per line:
      16. (1) library (2) market (3) park (4) cinema
      17. (1) puzzled (2) annoyed (3) excited (4) bored
    These lines are required — the question-extraction step downstream PARSES them to populate each question's 4-option array. Without them, options get hallucinated. The display cleanup step (cleanVocabClozePassageOcr) strips this block out of the version shown to the kid in the quiz UI, so duplication is NOT a concern — emit the options block here.
    Inside the passage itself, do NOT put "(16)", "(17)" etc next to the blanks — the question number is rendered by the UI from the question card, not the passage.
...
Output ONLY the clean passage/question text, no commentary.` });
          ...
          // tiers are 504-ing.
          //
          // EXCEPTION: 语文应用 MCQ (Chinese §1, Q1-14/15) gets
          // gemini-3.1-pro-preview as PRIMARY. 2.5-pro was still
          // making character-level transcription mistakes on the
          // emphasised tested phrase (the entire point of these
          // questions). 3.1-pro-preview is ~1.5-2× the cost per
          // token but more accurate on Chinese characters; for one
          // section per master paper the dollar delta is trivial.
          const isLangAppMcq = isChineseBooklet && (secLabel.includes("语文应用") || secLabel.includes("语文运用"));
          const OCR_MODELS = isLangAppMcq
...
- stem: the full question text.${secLabel.toLowerCase().includes("comprehension") && secLabel.toLowerCase().includes("open") ? ` For Comprehension OEQ: preserve ALL formatting including line breaks, tables, and checkboxes. Do NOT join into one sentence.` : ` If the OCR text wraps across multiple lines, JOIN them into one sentence. Do NOT preserve line breaks within a question stem.`}${secLabel.toLowerCase().includes("vocabulary cloze") ? `
  For Vocabulary Cloze MCQ: the stem MUST include the sentence from the passage that contains the underlined word. The student is picking a synonym for the underlined word — they are NOT filling in a blank. Wrap the underlined word in the marker syntax \`**(N)WORD**\` where N is the question number and WORD is the actual word from the sentence. E.g. for question 11 about the word "happy" in "The boy was happy when he saw the gift.", output: "The boy was **(11)happy** when he saw the gift." Do NOT use underscores or blanks.` : ""}${isMcqSection ? `
...
- errorWord: the underlined/erroneous word the student must correct` : ""}${secLabel.toLowerCase().includes("synthesis") ? `
  ...
  - STRIP THE LEADING QUESTION NUMBER FROM THE STEM. The questionNum field already carries the digit ("1", "2", "13"…). The stem MUST start with the actual question text — never "1." / "1．" / "1、" / "(1)" / "1．选出…" / "Q1 …". If the OCR line reads "1．他做事很__马虎__。" the stem you emit must read "他做事很__马虎__。" — drop the "1．". This applies to EVERY Chinese question, not just 语文应用 MCQ.
...
Return ONLY valid JSON:
{
  "questions": [
    ...
    `{"questionNum": "${prefix}${secFirstQ}", "stem": "What does the word 'happy' mean in paragraph 2?", "answer": null, "marksAvailable": 2, "pageIndex": 0, "yStartPct": 10, "yEndPct": 25, "syllabusTopic": "${sec.name}"}`}
          ...
          //
          // 语文应用 MCQ uses 3.1-pro-preview first (matching OCR_MODELS
          // above) so the emphasised tested phrase survives BOTH passes.
          const TEXT_EXTRACT_MODELS = isLangAppMcq
            ? (["gemini-3.1-pro-preview", "gemini-2.5-pro", "gemini-2.5-flash"] as const)
            : (["gemini-2.5-pro", "gemini-3.1-pro-preview", "gemini-2.5-flash"] as const);
