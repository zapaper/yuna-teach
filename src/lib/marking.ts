...
function englishMarkingRules(subject: string | null | undefined): string {
  ...
  - KEYWORD HANDLING — RECONSTRUCT BEFORE MARKING:
      The given word/phrase is PRINTED on the question paper between two blanks (the student doesn't write it, they fill the blanks around it). When the AI transcribes the student's handwriting, the printed keyword is naturally absent from the captured text — that is EXPECTED, not an error. Treat the keyword as known context that you SPLICE IN before marking.
      Algorithm:
      1. Find where the given word/phrase appears in the answer key. Note the words IMMEDIATELY before and immediately after it.
      2. Read the student's transcribed sentence.
        - If the given word/phrase is ALREADY present in the transcription (student wrote it explicitly), mark as written.
        - Otherwise SPLICE the given word/phrase into the student's sentence at the position the answer key uses (between the "before" word and the "after" word). The spliced sentence is what you mark — do NOT treat the missing keyword as an error.
      3. Compare the spliced (or as-written) sentence against the answer key:
        - If grammatically + semantically equivalent → FULL marks. Do NOT deduct for the "missing" keyword — the keyword was the printed connector, not something the student had to produce.
        - If the rewrite has actual errors (wrong tense, changed meaning, additional missing words, wrong word order beyond the connector position) → award marks per the standard "wrong / partial" judgement.
      Real example: keyword "when", expected "Everyone was amazed when Rahim won the race.", student wrote "Everyone was amazed Rahim won the race." → splice "when" between "amazed" and "Rahim" → reconstructed = "Everyone was amazed when Rahim won the race." = expected → FULL marks (2/2). No deduction.
