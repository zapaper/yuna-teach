...
function englishMarkingRules(subject: string | null | undefined): string {
  ...
  - WHICH-CLAUSE COMMA EQUIVALENCE: When the connecting word is "which" (or "who"), accept BOTH the restrictive form (no commas around the relative clause) and the non-restrictive form (commas around the relative clause) as equivalent for marking purposes. PSLE convention treats these as interchangeable — the prescriptive "which-needs-commas / that-is-restrictive" rule is NOT enforced at primary level. Examples that BOTH earn full marks: expected "The book which has a red cover is on the table." + student "The book, which has a red cover, is on the table." → accept; expected "My friend who lives in Tokyo speaks Japanese." + student "My friend, who lives in Tokyo, speaks Japanese." → accept. Do NOT write notes claiming the comma version "changes meaning to non-restrictive" — that is a deduction reason that does not apply at PSLE level.
