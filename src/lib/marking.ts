...
function englishMarkingRules(subject: string | null | undefined): string {
  ...
  - **READ THE LINE AS ONE SENTENCE (REPLACES THE OLD "splice the keyword" ALGORITHM)**:
      The synthesis row contains a mix of PRINTED BLACK text (a connecting keyword the student does NOT write) and BLUE INK handwriting (the parts the student fills in around the keyword). The printed keyword can sit anywhere on the row — BEFORE the first blank, BETWEEN two blanks, AFTER the final blank, or any combination.
      ...
      Steps:
      1. Identify EVERY printed keyword fragment on the answer row (in black, distinct from the blue handwriting). Note each one literally with its visible position — e.g. "between blank-1 and blank-2: 'because of Nisa's'", or "trailing (after the only blank): 'by the chefs.'", or "leading (before the only blank): 'Although'".
      2. Read the row left-to-right. Output the FULL sentence as it would read off the page: blue ink + printed black keyword(s), in their actual positions.
         · BETWEEN example: blue "The salesgirl replaced the vase", printed "because of Nisa's", blue "complaint that it had a crack." → studentAnswer = "The salesgirl replaced the vase because of Nisa's complaint that it had a crack."
         · TRAILING example: blue "The food for tonight's wedding banquet is being prepared", printed "by the chefs." → studentAnswer = "The food for tonight's wedding banquet is being prepared by the chefs."
         · LEADING example: printed "Although", blue "it was raining, we went out." → studentAnswer = "Although it was raining, we went out."
  ...
  - PARENTHETICAL COMMA TOLERANCE: When a phrase can be set off by commas without changing the meaning, accept BOTH forms (with or without commas) as equivalent. The prescriptive "restrictive needs no commas / non-restrictive needs commas" distinction is NOT enforced at PSLE level. Applies to:
    · Relative clauses with "which" / "who": expected "The book which has a red cover is on the table." + student "The book, which has a red cover, is on the table." → accept; expected "My friend who lives in Tokyo speaks Japanese." + student "My friend, who lives in Tokyo, speaks Japanese." → accept.
    · Prepositional aside phrases — "except (for) X", "apart from X", "including X", "besides X", "aside from X", "such as X". Expected "All the children except Tom wanted to go to the zoo." + student "All the children, except for Tom, wanted to go to the zoo." → accept full marks. Expected "Everyone, apart from Mary, agreed." + student "Everyone apart from Mary agreed." → accept full marks. The presence or absence of commas around these phrases is a punctuation choice, not a meaning change.
    Do NOT write notes claiming the comma version "changes meaning to non-restrictive" or that the phrase is "essential / restrictive" — that is a deduction reason that does NOT apply at PSLE level for any of the constructions above.
...
Concretely:
  ...
  - "STATE HOW / EXPLAIN HOW / DESCRIBE HOW X HAPPENS" QUESTIONS: the phrase "X happens / forms / occurs / is produced" is the question's framing — it tells the student WHAT the answer is about. It is NOT a marking point. The mechanism / cause the student gives IS the answer. Example: stem "State how a shadow is formed." + key "A shadow is formed when light is partially or completely blocked by an object." + student "Light is partially or completely blocked by the opaque object." → ACCEPT for full credit. The phrase "a shadow is formed" was given by the stem; the new content the student must produce is "light is blocked by the object", and they did. Do NOT deduct because they didn't write "a shadow is formed" — the question already established that. Same for "State how the boat moves" / "Describe how the seed is dispersed" / "Explain how sound travels" / etc.
  ...
  if (useGpt54PrimaryRemark) {
    ...
    try {
      const r = await runOpenAIFallback(
        ...
        `remark-q${question.questionNum}-circuit-gpt54`,
      );
      text = r.text ?? "";
      if (!text) throw new Error("Empty gpt-5.4 response");
    } catch (err) {
      console.warn(`[marking] Q${question.questionNum} gpt-5.4 call failed — falling back to Gemini:`, err instanceof Error ? err.message : err);
      const response = await withTimeout(
        getAI().models.generateContent({
        ...
        GEMINI_TIMEOUT_MS,
        `remark question ${questionId} (Gemini fallback)`
      );
      text = response.text ?? "";
...
// path. Set to 0: ANY question that the marker couldn't put a verdict
// on (Gemini + OpenAI all failed) keeps the paper at status=failed so
// the parent knows it needs a re-mark. Partial / "almost complete"
// papers are misleading — the score the kid sees doesn't reflect the
// real total until every question has been visited.
const MAX_UNMARKED_FOR_CAVEAT = 0;
...
async function _markExamPaperOnce(paperId: string): Promise<void> {
  ...
  try {
    ...
    async function markBatch(
      ...
      // Gemini. Used by the science electrical-circuit path as sole
      // primary (gpt-5.4 has been more accurate than gemini-3.1-pro
      // on hand-drawn circuit topology). Returns [] if the call
      // fails so the caller can fall back to Gemini.
      useOpenAI?: boolean
          ...
          const croppedResults = await Promise.all(
            writtenQs.map(async (q) => {
              try {
                ...
                // ── Science electrical-circuit: gpt-5.4 primary ──
                // gpt-5.4 has been consistently more accurate (and
                // faster) than gemini-3.1-pro-preview on these
                // drawings — both on topology (series vs parallel)
                // and on the fiddly details (terminal connections,
                // '+' label placement) that pro Gemini tends to
                // accept loosely. So for science drawable electrical
                // circuits we route directly to gpt-5.4 and skip the
                // Gemini call entirely. Falls back to Gemini if the
                // OpenAI key isn't set so the path is never broken
                // in test/dev.
                ...
                const initial = await markBatch(
                  croppedBase64, [q],
                  `page ${pageIndex} Q${q.questionNum} (cropped${useGpt54PrimaryExam ? " gpt-5.4" : ""})`,
                  true, modelOverride, extraContext,
                  useGpt54PrimaryExam, // useOpenAI
                );
                // Safety net: if gpt-5.4 returned nothing for any
                // reason (transient 5xx, quota), fall back to Gemini
                // so the question still gets a verdict.
                if (useGpt54PrimaryExam && initial.length === 0) {
                  console.warn(`[marking] Q${q.questionNum} gpt-5.4 returned empty — falling back to Gemini`);
                  const fallback = await markBatch(croppedBase64, [q], `page ${pageIndex} Q${q.questionNum} (cropped Gemini fallback)`, true, modelOverride, extraContext);
                  if (fallback.length > 0) initial.push(...fallback);
                }
