...
【词汇与句子两轴的评分原则 — 重要】
错别字 (例如 "红彤彤地" → "红彤彤的") 和单字遗漏已经在错别字检查 (wrongWords) 阶段被独立标出。**词汇分和句子分不应再次扣这些机械性小错误**。否则就是 "同一个错被扣两次"。

**词汇分 (Vocabulary) 看的是"丰富度"和"切合度"**:
- 用到的成语 / 好词好句的数量 + 是否切合上下文 + 描写是否生动具体。
- 8-9 分: 用了 5+ 个成语且大多数切合情境，有生动描写。**1-2 处词语搭配不当不应该扣到 8 分以下**。
- 10 分: 6+ 个成语全部贴切，描写丰富有层次。
- 6-7 分: 成语少于 3 个，描写干瘪。
- ≤ 5 分: 通篇都是简单常用词，几乎没有任何成语 / 好句。

**句子分 (Sentence) 看的是"流畅度"和"结构变化"**:
- 段落过渡是否顺、句子有没有变化、整体读起来流不流畅。
- 8-9 分: 整体叙述流畅、段落过渡自然，**1-2 处"的/地/得"混用或单一语病不应该扣到 8 分以下**。
- 10 分: 句式有变化 (长短结合、有问句 / 感叹句 / 转折句)，整体如行云流水。
- 6-7 分: 多处语病 / 句意不清 / 段落转换生硬。
- ≤ 5 分: 通篇句子结构单一、多处不通顺、读起来很卡。

**重要**: 13 处错别字大多是 "的/地/得" 这类机械混用，不能让词汇分和句子分双重扣分到 6-7。这种作文如果成语用得丰富、叙述流畅，词汇分应该 8-9，句子分应该 7-8 (因为机械错误略多)。
