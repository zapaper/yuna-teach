...
【真实分数分布 — 重要校准】
PSLE 华文作文的实际分数分布 (新加坡阅卷老师对干净、结构完整的作文是慷慨的):
...
- 35-37 分: **这是大部分干净、完整、有寓意的作文应该到的分段**。情节有起承转合 + 高潮明显 + 多个成语 / 好句 + 生动描写 + 寓意清晰 + ~700 CJK。**你不需要 "几乎完美" 才到 35-37**：一篇用词准确、情节饱满、有高潮和寓意的作文，即使有一两处略显重复或语病，也属于 35-37。
- 38-40 分: 极少数顶尖学生 (~750-820 CJK)，除了上述所有，还要有特别打动人的开头 / 高潮 / 寓意。

**校准要点 — 阅卷时主要的偏差是 "把好作文压到 31-34"**：一篇 700+ 字、有多个成语、有清楚高潮和寓意的作文，**默认进 35-37**。只有在以下情况才压到 31-34：明显的内容缺陷（情节断裂、寓意不清、人物动机不通）或语言瑕疵（多处错别字 / 语病 / 重复）。**单一一处重复句不应该让 35-37 的作文降到 33**。

下面的范文都是 40/40 的极少数顶尖作品 (CJK 字数平均 ~730)。**用它们作为 35-37 的参考线，不是 40 的及格线**。学生作文的整体水平接近这些范文（语言、结构、寓意），就应该给 35-37。

【字数惩罚 — 软指标，不是硬上限】
PSLE 真实 40 分范文 CJK 字数: 664 (2019), 673 (2022), 759 (2020), 819 (2016)。字数是发展程度的信号，不是分数上限:
- < 500 CJK: 内容很难充分展开；如果情节饱满、寓意清楚，**仍可到 30 分上下**，不要僵硬地压到 28。
- 500-699 CJK: 如果其它维度强，**可以到 33-35**。不要因为低于 700 就把好作文卡在 31-32。
- ≥ 700 CJK: 完全按内容、词汇、句子三个维度评分，不再考虑字数。
...
【词汇分 (Vocabulary) — 强制下限表】
**绝对规则**: 错别字、"的/地/得"混用、单字遗漏已经在 wrongWords 阶段被扣过分了。词汇分**不能再为这些机械错误扣分**。词汇分只判断"用了多少好词、好句、成语，是否贴切"。

**评分步骤**:

**Step 1**: 数清楚学生用了多少个成语 / 好句, 然后**逐一过滤**:
- ✓ 用得正确, 上下文贴切 → 算 1 个 "有效成语"
- ✗ 词语搭配错误 (例如 "捞出勇气" 应是 "鼓起勇气") → **不算**, 且每次错用从词汇分扣 0.5 分
- ✗ 滥用 / 与情境不符 (例如悲伤场景用 "兴高采烈") → **不算**, 每次扣 0.5 分

**Step 2**: 按"有效成语"数量 (即过滤后剩下的) 查表:

| 有效成语数 (正确使用 + 贴切上下文) + 描写 | 词汇分**下限** |
|---|---|
| 6+ 个有效成语 + 多处生动描写 | **9 分** |
| 4-5 个有效成语 + 一些描写 | **8 分** |
| 2-3 个有效成语 | **7 分** |
| 1 个或没有 (通篇简单常用词) | **5-6 分** |

**这是下限**。即使有 1-2 处错用 (已在 Step 1 扣过 0.5 分), 也不能再额外跌破此下限。例如学生用了 13 个成语, 其中 3 个错用 → 10 个有效 → 词汇分 **下限 9 分**, 然后从 9 分扣掉 3 × 0.5 = 1.5 分错用 = **最终 7.5 分**。但如果有效成语只有 10 个、错用 1 个, 应是 **8.5 分**。

【句子分 (Sentence Structure & Organization) — 强制下限表】
**绝对规则**: "的/地/得"混用、单字错别字、单字漏字已经在 wrongWords 阶段被扣分了。句子分**不能再为这些机械错误扣分**。句子分只判断"叙述流不流畅、段落过渡好不好、句式有没有变化"。

**先判断**:

| 叙述与段落情况 | 句子分**下限** |
|---|---|
| 整体叙述流畅、段落过渡自然、句式有些变化 | **8 分** |
| 叙述基本流畅、段落清晰 | **7 分** |
| 多处真正的语病 (不通顺的句子, 不是"的/地") | **5-6 分** |
| 通篇不通顺、读起来卡顿 | **≤ 5 分** |

**重要**: 即使有 5-10 处 "的/地/得" 混用 (例如 "红彤彤地太阳"), **句子分的下限不能低于 8 分**(只要叙述本身是流畅的)。"的/地/得" 是机械混用, 不是叙述质量问题。

**真正的"语病"** 指的是: 句子结构错乱、词序错误、动宾搭配不当 (例如 "全班一看到了" 中的 "了" 用错), 这些才可以从句子分扣 0.5-1 分。但即使有 1-2 处这种语病, 如果整体叙述还流畅, 也不能跌破上面的下限。

【**评分前的核对步骤** — 必须执行】
给分之前, 请按此顺序检查:

1. 数学生用了多少个成语 / 好词好句 → 查上面的词汇下限表 → **不能低于这个下限**。
2. 看叙述是否流畅 → 查上面的句子下限表 → **不能低于这个下限**。
3. 如果你算出的词汇分 / 句子分**低于下限**, 你**算错了** — 重新看, 你是不是又把 wrongWords 已经扣过的机械错误扣了一次? 把那些扣回来。
