      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            ...
            title="Pick 2-10 essays and get a cross-essay coaching summary"
          >
          ...
          <div className="space-y-2">
            {rows.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className={`relative bg-white border rounded-xl p-4 group transition-colors ${
                    batchMode && isPicked ? "border-violet-400 ring-2 ring-violet-200"
                    : "border-slate-200 hover:border-slate-400"
                  }`}
                >
                  <div className="flex items-start gap-3">
                    {/* Batch-mode checkbox — only on ready rows. */}
                    {batchMode && (
                      <div className="pt-1">
                        {isReady ? (
                          <input
                    ...
                    <div className="flex-1">
                      {/* Link wraps everything when not in batch mode;
                          batch mode replaces the link with click-to-toggle. */}
                      {batchMode ? (
                        <button
                          type="button"
                          onClick={() => isReady && !capReached && toggleBatchPick(r.id)}
                          disabled={!isReady || (capReached && !isPicked)}
...
// Extracted to keep the list-item JSX readable in both link mode and
// batch-pick mode without duplicating the entire row body.
function AttemptRowBody({ r }: { r: AttemptRow }) {
  return (
    <div className="flex justify-between items-start">
      <div className="min-w-0">
        ...
        <div className="text-xs text-slate-500 mt-1">
          ...
          {r.englishComponent && <> · {r.englishComponent}</>}
          {!r.language && r.optionType && <> · {r.optionType}</>}
        </div>
      </div>
          ...
          return (
            <div key={bi} className={`border ${palette.border} rounded-xl overflow-hidden`}>
              ...
              <div className="px-4 py-3 space-y-3 bg-white">
                {b.advice.map((a, ai) => (
                  <div key={ai}>
                    <div className="text-sm font-bold text-slate-900">{a.tip}</div>
