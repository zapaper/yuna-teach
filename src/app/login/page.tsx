...
// Detect Capacitor at runtime so we can hide OAuth buttons inside
// the iOS app — see the OAuth section below.
...
function LoginContent() {
  ...
  // OAuth via Google / Apple only works inside a browser that
  // shares a cookie jar with the page that started the flow. The
  // iOS Capacitor shell sets `limitsNavigationsToAppBoundDomains:
  // true`, which forces the OAuth redirect to open in external
  // Safari — and external Safari can't see the pkce/state cookies
  // we just wrote inside WKWebView. Result: every attempt errors
  // with `InvalidCheck: pkceCodeVerifier value could not be parsed`
  // on the callback. Until we wire up ASWebAuthenticationSession
  // through a native plugin, hide the buttons on iOS native — the
  // user can still sign in with their email + password. Detection
  // runs after mount (Capacitor returns false during SSR), so the
  // buttons render briefly then disappear; mounted-flag gates the
  // initial render so they never flicker in.
  const [oauthAvailable, setOauthAvailable] = useState(false);
  useEffect(() => {
    setOauthAvailable(!Capacitor.isNativePlatform());
  }, []);
    ...
    <div className="bg-surface font-body text-on-surface min-h-screen flex flex-col lg:flex-row items-center justify-center p-6 lg:p-12 relative overflow-hidden gap-8 lg:gap-16">
      ...
      <main className="w-full max-w-[480px] z-10 lg:shrink-0">
        ...
        <div className="bg-surface-container-lowest rounded-[2rem] p-8 md:p-12 border border-white/40" style={{ boxShadow: "0 20px 40px rgba(11,28,48,0.06)" }}>
          ...
          {/* ── OAuth providers (hidden on iOS native — see useEffect above) ── */}
          {oauthAvailable && (
            <div className="mt-8">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button
                  type="button"
                  onClick={() => signIn("apple", { redirectTo: "/post-login" })}
