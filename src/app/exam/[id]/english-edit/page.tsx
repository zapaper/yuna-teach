...
export default function EnglishEditPage({
  params,
}: {
...
function EnglishEditContent({ id }: { id: string }) {
  ...
  // English-only guard. /english-edit is the language-isolated clone of
  // /edit; if someone routes a math/science/chinese paper here by
  // mistake, kick them back to the canonical /edit. Keeps the languages
  // hard-separated per the "languages must be kept separate" rule.
  if (!(paper.subject ?? "").toLowerCase().includes("english")) {
    return (
      <div className="p-6 text-center py-24">
        <p className="text-slate-500">This page is English-only. This paper is &ldquo;{paper.subject}&rdquo; — use the regular Edit page instead.</p>
        <button
