// POST /api/essay-coach/[id]/phrase-alternatives
//
// Parent-side mirror of the admin route. Given a selected phrase + its
// surrounding paragraph, ask Gemini for 3-5 context-fit alternatives
// the kid can swap in. Used by the right-click menu on the detail
// page's editable essay.
...
export async function POST(
  ...
  const row = await prisma.compoAttempt.findUnique({
    where: { id },
    select: { studentTopic: true, uploaderId: true },
  });
...
只输出 JSON，不要 markdown，不要解释。`;
  ...
  try {
    ...
    }, 2, 3000, "essay-coach-phrase-alternatives");
    ...
    try {
      ...
      console.warn(`[essay-coach:${id}] phrase-alternatives JSON parse failed, salvaging:`, (parseErr as Error).message);
      ...
      console.warn(`[essay-coach:${id}] phrase-alternatives salvaged ${alts.length} entry(s)`);
    }
    return NextResponse.json({ alternatives: alts });
  } catch (err) {
    console.error(`[essay-coach:${id}] phrase-alternatives failed:`, err);
